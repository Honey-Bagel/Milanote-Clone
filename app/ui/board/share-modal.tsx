...
export default function ShareModal({ boardId, isOpen, onClose }: ShareModalProps) {
	...
	return (
		<div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
			<div ref={modalRef} className="bg-[#0f172a] border border-white/10 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col text-foreground">
				{/* Header */}
				<div className="p-6 border-b border-white/10 flex-shrink-0 flex items-center justify-between">
					...
					<button 
						className="text-secondary-foreground hover:text-white hover:bg-white/5 p-2 rounded-lg transition-colors" 
						onClick={onClose}
					>
						<X className="w-5 h-5" />
					</button>
				...
				<div className="p-6 overflow-y-auto flex-1 space-y-6 custom-scrollbar">
					...
					{isOwner && (
						<div>
							<label className="block text-sm font-bold text-secondary-foreground uppercase tracking-wider mb-4">
								Invite People
							</label>
							<div className="flex gap-2">
								<input
									...
									className="flex-1 bg-[#020617] border border-white/10 rounded-lg px-4 py-3 text-sm text-white placeholder-slate-500 focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all"
								/>
								<select
									value={selectedRole}
									onChange={(e) => setSelectedRole(e.target.value as BoardRole)}
									className="bg-[#020617] border border-white/10 rounded-lg px-4 py-3 text-sm text-foreground focus:ring-2 focus:ring-primary focus:border-transparent outline-none cursor-pointer"
								>
								...
								<button
									...
									disabled={isAdding}
									className="bg-primary hover:bg-primary text-white px-6 py-3 rounded-lg text-sm font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-primary/20"
								>
					...
					{collaborators.length > 0 && (
						<div>
							<div className="flex items-center gap-2 mb-4">
								...
								<h3 className="text-sm font-bold text-secondary-foreground uppercase tracking-wider">
									...
									>
										...
										{isOwner ? (
											<select
												...
												className="text-xs text-foreground bg-[#0f172a] border border-white/10 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary flex-shrink-0 cursor-pointer hover:border-white/20 transition-all"
											>
												<option value="editor">Can edit</option>
												<option value="viewer">Can view</option>
											...
											<span className="text-xs text-secondary-foreground bg-white/5 px-3 py-1.5 rounded-lg border border-white/10 capitalize flex-shrink-0">
												{collab.role}
											</span>
										)}
									</div>
							...
							<div className="mt-3 ml-1">
								<label className="flex items-center gap-2 cursor-pointer group">
									<div className="relative">
										...
										/>
										<div className="w-5 h-5 border-2 border-white/20 rounded bg-[#020617] peer-checked:bg-primary peer-checked:border-indigo-600 transition-all flex items-center justify-center">
					...
					{board?.is_public && board.share_token && (
						<div>
							<h3 className="text-sm font-bold text-secondary-foreground uppercase tracking-wider mb-4">
								Public Share Link
							</h3>
							<div className="flex items-center gap-2 p-4 bg-[#020617] border border-white/10 rounded-xl">
								<LinkIcon className="text-muted-foreground w-4 h-4 flex-shrink-0" />
								<input
									type="text"
								...
								<button
									...
									className="text-primary hover:text-primary font-medium text-sm flex items-center gap-2 flex-shrink-0 px-3 py-1.5 hover:bg-primary/10 rounded-lg transition-all"
								>
									{copied ? (
										<>
							...
							{isOwner && (
								<div className="mt-3 flex items-center justify-end">
									<button
										...
										disabled={sharingLoading}
										className="text-secondary-foreground hover:text-white text-sm font-medium disabled:opacity-50 flex items-center gap-2 hover:bg-white/5 px-3 py-1.5 rounded-lg transition-all"
									>
						...
						<div className="bg-[#0f172a] border border-white/10 rounded-2xl shadow-2xl p-6 max-w-md w-full">
							...
							<div className="flex justify-end gap-3">
								<button
									...
									className="px-4 py-2.5 text-foreground hover:bg-white/5 rounded-lg font-medium transition-all border border-white/10"
								>
									Cancel
								</button>
								<button
